...
const mcp = new Server(
  { name: 'telegram-unleashed', version: '1.1.0' },
  {
    ...
    instructions: [
      ...
      '',
      'In a group, only the finished message belongs to the group. The status line and the activity feed are redirected to the operator when statusChatId is configured, so do not compensate by narrating progress into the group yourself — no "let me check that", no running commentary, no thinking out loud. Send one message when you have something to say. Everything the group did not ask for is noise to everyone in it.',
      '',
